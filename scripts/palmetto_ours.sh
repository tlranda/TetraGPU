...
# See {avp}100.sh for different GPU models to call here (should also handle cd/source env.sh)
...
mkdir -p ${HOSTNAME_OVERRIDE}_outputs;
...
for ds in ${datasets[@]}; do
    ...
    cmd="build_vm-slurm-p-login03.palmetto.clemson.edu/./main --input ${ds} --arrayname ${used_array} -p _index -d 0 -t ${full_subscribe} --export /dev/null";
	...
	gpu_count=$((2 ** ${gpu_count}));
        ...
        python3 processours.py ${HOSTNAME_OVERRIDE}_outputs/${shortname}_${gpu_count}_GPUS*.output;
    done;
done;
